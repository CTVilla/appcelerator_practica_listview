...
function apiCall(_options, _callback) {
	...
	if (Ti.Network.online && !_options.localOnly) {
		...
		xhr.open(_options.requestMethod, _options.url);
		...
		console.log("_options.data: "+JSON.stringify(_options.data));
		xhr.send(JSON.stringify(_options.data));
	} else {
		// we are offline
		_callback({
			success : false,
...
function Sync(method, model, opts) {
	...
	_.defaults(params, {
		...
		// xhr settings
		timeout : model.config.timeout || 7000,
		cache : false,
